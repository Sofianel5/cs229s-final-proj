...
class GPT(nn.Module):
    ...
    def heuristic(self, model, current_idx, probs, idx_next, temperature):
        if probs is None or current_idx.nelement() == 0 or len(current_idx.size()) < 2:
            # Initial heuristic value when starting or in case of empty current_idx
            mu = 0
            sigma = 1
            return np.random.normal(mu, sigma)
        else:
            # Incrementing iteration counter for debugging or analysis
            self.iter += 1
            #print("Heuristic Iteration:", self.iter)

            # Extracting the probability of the next token
            token_prob = probs[0, idx_next].item()

            # Adjusting for sequence length
            positional_factor = np.sqrt(current_idx.size(1) + 1)

            # Calculating the probability-based heuristic value
            probability_heuristic_value = -np.log(token_prob + 1e-9)
            heuristic_value = 0.7 * probability_heuristic_value / positional_factor

            # Adding a controlled random adjustment
            random_adjustment = np.random.normal(0, 0.3)

            # Combining the adjusted heuristic value with the random factor
            combined_value = heuristic_value + random_adjustment
    ...
    def astar_generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        def process_node(oracle_model, prob, idx_next, current, temperature, cache_probs):
            ...
            new_cost = current.cost - torch.log(prob) + self.heuristic(oracle_model, current_idx, cache_probs, idx_next, temperature)
            # Pruning based on heuristic value
            #print(new_cost)
            heuristic_threshold = 17.5  # PRUNING VALUE
            if new_cost > heuristic_threshold: # PRUNING
        ...
        initial_heuristic_value = self.heuristic(oracle_model, current_idx, None, idx_next, temperature)
        ...
        while not open_set.empty():
            ...
            if current_idx.size(1) >= max_new_tokens:
                ...
                if len(sequence) > 0:
                    ...
                    print("No sequence generated. Consider adjusting pruning threshold.")
                    return None
            ...
            with ThreadPoolExecutor() as executor:
                futures = [executor.submit(process_node, oracle_model, prob, idx_next, current, temperature, probs)
        ...
        ## BACKUP PLAN (no guaranteed sequence)
        sequence = []
        while current is not None:
            sequence.append(current.idx)
            current = current.parent
