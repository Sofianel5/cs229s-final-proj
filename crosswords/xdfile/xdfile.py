...
#                log("%s[%s] '%s' -> '%s'" % (self.filename, fieldname, self.headers[fieldname], newvalue))
    ...
    def to_unicode(self, emit_clues=True, show_solved=True):
        # headers (section 1)
        ...
        if show_solved:
            ...
            r += EOL.join([re.sub('[A-Z]', OPEN_CHAR, row) for row in self.grid])
        r += EOL + EOL + EOL
        ...
        if emit_clues:
            ...
            for pos, clue, answer in self.clues:
                ...
                r += "%s%s. %s ~ %s" % (cluedir, cluenum, (clue or "[XXX]").strip(), (answer if show_solved else "?"))
                r += EOL
